...
func main() {
	...
	// 読み込みバッファの作成
	audioData := make([]int16, bufferSize*int(header.NumChannels))

	// バッファの作成
	monoAudioData := make([]int16, bufferSize*1)

	// 再生ストリームの作成
	stream, err := portaudio.OpenDefaultStream(0, 1, float64(header.SampleRate), bufferSize, &monoAudioData)
	if err != nil {
	...
	for {
		...
		monoAudioData = stereoToMono1(audioData)
