...
func LoadHeader(file io.Reader) (*WAVHeader, error) {
	...
	fmt.Println("ChunkID:", string(h.ChunkID[:]))

	// チャンクサイズの読み取り（使用しない）
	if err := binary.Read(file, binary.BigEndian, &h.ChunkSize); err != nil {
		fmt.Printf("Failed to read chunkSize: %v", err)
		return nil, err
	}

	// フォーマットのチェック
	if err := binary.Read(file, binary.BigEndian, &h.Format); err != nil {
	...
	fmt.Println("Format:", string(h.Format[:]))

	// fmtチャンクのチェック
	if err := binary.Read(file, binary.BigEndian, &h.Subchunk1ID); err != nil {
	...
	fmt.Println("Subchunk1ID:", string(h.Subchunk1ID[:]))

	// fmtチャンクサイズの読み取り（使用しない）
	if err := binary.Read(file, binary.BigEndian, &h.Subchunk1Size); err != nil {
		fmt.Printf("Failed to read subchunk1Size: %v", err)
		return nil, err
	}

	// オーディオフォーマットの読み取り
	if err := binary.Read(file, binary.BigEndian, &h.AudioFormat); err != nil {
		fmt.Printf("Failed to read audioFormat: %v", err)
		return nil, err
	}

	// チャンネル数の読み取り
	if err := binary.Read(file, binary.BigEndian, &h.NumChannels); err != nil {
		fmt.Printf("Failed to read numChannels: %v", err)
		return nil, err
	}
	// サンプリングレートの読み取り
	if err := binary.Read(file, binary.BigEndian, &h.SampleRate); err != nil {
		fmt.Printf("Failed to read sampleRate: %v", err)
		return nil, err
	}

	// バイトレートの読み取り（使用しない）
	if err := binary.Read(file, binary.BigEndian, &h.ByteRate); err != nil {
		fmt.Printf("Failed to read byteRate: %v", err)
		return nil, err
	}

	// ブロックアラインの読み取り（使用しない）
	if err := binary.Read(file, binary.BigEndian, &h.BlockAlign); err != nil {
		fmt.Printf("Failed to read blockAlign: %v", err)
		return nil, err
	}

	// サンプルあたりのビット数の読み取り
	if err := binary.Read(file, binary.BigEndian, &h.BitsPerSample); err != nil {
		fmt.Printf("Failed to read bitsPerSample: %v", err)
		return nil, err
	}

	// dataチャンクのチェック
	if err := binary.Read(file, binary.BigEndian, &h.Subchunk2ID); err != nil {
